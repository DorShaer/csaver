...
class Crawler:
    def __init__(self, base_url: str, har_file: str):
        """
    ...
    async def report_protected_resource(self, link: str, trigger_cause: str, resp: aiohttp.ClientResponse, redirect_details: Dict[str, str]=None):
        """
        Report detailed info about a protected or redirecting resource.
        """
    ...
    async def process_link(self, link: str, headers: dict, cookies: dict, idx: int, total: int):
        ...
        - Make a GET request
        - If non-200, report protected resource
        - If 200 and text/html, check for client-side redirect patterns
        """
        print(colored(f"Processing {idx}/{total}: {link}", "blue"), flush=True)

        # Not using headers/cookies for the request, to emulate no-auth scenario
        async with self.session.get(link, allow_redirects=False) as resp:
            if resp.status != 200:
                ...
                # Attempt to check for client-side redirects if the content is HTML/text
                ...
                if 'html' in content_type or 'text' in content_type:
                    ...
                    # Binary or other non-text content: skip checking for client-side redirect
                    pass
...
if __name__ == '__main__':
    ...
    crawler = Crawler(base_url, har_file)
